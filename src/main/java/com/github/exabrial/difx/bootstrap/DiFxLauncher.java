...
/**
 * Entry point a DiFx application extends. Calling {@link #launch(String...)} bootstraps a CDI SE container, applies any
 * customization via {@link #configureContainer(SeContainerInitializer)}, then hands control to JavaFX by launching the
 * {@link #applicationClass()}. The container is owned for the entire JavaFX lifecycle and closed when the application
 * exits. This class is instantiated with {@code new}, before any container exists, so it is deliberately not a CDI bean.
 */
public abstract class DiFxLauncher {
	 ...
	 * alternatives, enable interceptors, disable discovery, set properties. The default applies nothing.
	 */
